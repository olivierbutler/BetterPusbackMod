...
static int compute_segs_impl(const vehicle_t *veh, vect2_t start_pos,
                             double start_hdg, vect2_t end_pos, double end_hdg,
                             list_t *segs, bool_t recurse);
...
static double straight_run_speed(const vehicle_t *veh, list_t *segs,
                                 double rmng_d, bool_t backward,
                                 const seg_t *next, bool_t *out_decelerating);

static int construct_segs_oblique(const vehicle_t *veh, vect2_t start_pos,
                                  double start_hdg, vect2_t midpt,
                                  double mid_hdg, vect2_t end_pos,
                                  double end_hdg, list_t *segs) {
  int n1, n2;

  n1 = compute_segs_impl(veh, start_pos, start_hdg, midpt, mid_hdg, segs,
                         B_FALSE);
  if (n1 == -1)
    return (-1);
  n2 = compute_segs_impl(veh, midpt, mid_hdg, end_pos, end_hdg, segs, B_FALSE);
  if (n2 == -1) {
    for (int i = 0; i < n1; i++) {
...
static int compute_segs_oblique(const vehicle_t *veh, vect2_t start_pos,
                                double start_hdg, vect2_t end_pos,
                                double end_hdg, list_t *segs, bool_t backward,
                                double radius) {
  ...
  for (int i = 0; i < 2; i++) {
    for (int j = 0; j < 2; j++) {
      vect2_t c1 = vect2_add(start_pos, vect2_scmul(vect2_norm(d1, i), radius));
      vect2_t c2 = vect2_add(end_pos, vect2_scmul(vect2_norm(d2, j), radius));
      vect2_t s = vect2_mean(c1, c2);
      vect2_t s2c1 = vect2_sub(c1, s);
      vect2_t s2c2 = vect2_sub(c2, s);
      double a;

      if (!IS_NULL_VECT(vect2vect_isect(s2c1, s, sv, start_pos, B_TRUE)) ||
          !IS_NULL_VECT(vect2vect_isect(s2c2, s, ev, end_pos, B_TRUE)))
        continue;
      ...
      if (!IS_NULL_VECT(vect2vect_isect(vect2_set_abs(vect2_rot(s2c1, a), 1e10),
                                        s, sv, start_pos, B_TRUE)) &&
          !IS_NULL_VECT(vect2vect_isect(vect2_set_abs(vect2_rot(s2c2, a), 1e10),
                                        s, ev, end_pos, B_TRUE))) {
        int n = construct_segs_oblique(
            veh, start_pos, start_hdg, s,
            dir2hdg(vect2_rot(backward ? s2c1 : s2c2, a)), end_pos, end_hdg,
            segs);
        if (n != -1)
          return (n);
      }
      ...
      if (!IS_NULL_VECT(
              vect2vect_isect(vect2_set_abs(vect2_rot(s2c1, -a), 1e10), s, sv,
                              start_pos, B_TRUE)) &&
          !IS_NULL_VECT(
              vect2vect_isect(vect2_set_abs(vect2_rot(s2c2, -a), 1e10), s, ev,
                              end_pos, B_TRUE))) {
        int n = construct_segs_oblique(
            veh, start_pos, start_hdg, s,
            dir2hdg(vect2_rot(backward ? s2c1 : s2c2, -a)), end_pos, end_hdg,
...
static int compute_segs_impl(const vehicle_t *veh, vect2_t start_pos,
   ...
   */
  min_radius =
      MAX(tan(DEG2RAD(90 - (veh->max_steer * SEG_TURN_MULT))) * veh->wheelbase,
          MIN_TURN_RADIUS);
  ...
  if (IS_NULL_VECT(turn_edge)) {
    if (recurse) {
      return (compute_segs_oblique(veh, start_pos, start_hdg, end_pos, end_hdg,
                                   segs, backward,
...
int compute_segs(const vehicle_t *veh, vect2_t start_pos, double start_hdg,
  ...
  return (compute_segs_impl(veh, start_pos, start_hdg, end_pos, end_hdg, segs,
                            B_TRUE));
}
 ...
 */
static vect2_t veh_pos2fixed_pos(const vehicle_pos_t *pos,
                                 const vehicle_t *veh) {
  if (veh->use_rear_pos) {
    return (
        vect2_add(pos->pos, vect2_scmul(hdg2dir(pos->hdg), veh->fixed_z_off)));
  }
  return (pos->pos);
}

static void drive_on_line(const vehicle_pos_t *pos, const vehicle_t *veh,
                          vect2_t line_start, double line_hdg, double speed,
                          double arm_len, double steer_corr_amp,
                          bool_t keep_aligned, double *last_mis_hdg, double d_t,
  ...
  if (veh->wheelbase < MIN_STEERING_ARM_LEN) {
    steer_corr_amp =
        fx_lin(veh->wheelbase, 0.5, 1, MIN_STEERING_ARM_LEN, steer_corr_amp);
  }
  ...
  printf("mis_hdg: %5.1f d_mis_hdg: %5.1f rhdg: %5.1f arm: %3.1f oc:%d "
         "st:%5.1f\n",
         mis_hdg, d_mis_hdg, rhdg, steering_arm, overcorrecting, steer);
...
static double next_seg_speed(const vehicle_t *veh, list_t *segs,
                             const seg_t *next, bool_t cur_backward) {
  if (next != NULL && next->backward == cur_backward) {
    if (next->type == SEG_TYPE_STRAIGHT) {
      return (straight_run_speed(veh, segs, next->len, next->backward,
                                 list_next(segs, next), NULL));
    } else {
      return (turn_run_speed(veh, segs, rel_hdg(next->start_hdg, next->end_hdg),
...
static double turn_run_speed(const vehicle_t *veh, list_t *segs, double rhdg,
  ...
  double spd =
      straight_run_speed(veh, segs, rmng_d, backward, next, out_decelerating);
...
static double straight_run_speed(const vehicle_t *veh, list_t *segs,
  ...
  case 1:
    ...
    break;
  case 2:
    spd = MIN(veh->max_decel * MAX(ts[0], ts[1]) + next_spd, cruise_spd);
    break;
  default:
    spd = next_spd;
    break;
...
static void turn_run(const vehicle_pos_t *pos, const vehicle_t *veh,
                     const seg_t *seg, double *last_mis_hdg, double d_t,
                     double speed, double *out_steer, double *out_speed) {
   ...
   */
  vect2_t c =
      vect2_add(vect2_set_abs(vect2_norm(hdg2dir(start_hdg), seg->turn.right),
                              seg->turn.r),
                seg->start_pos);
  ...
  drive_on_line(pos, veh, r, hdg, speed, veh->wheelbase / 5, 3, B_TRUE,
                last_mis_hdg, d_t, out_steer, out_speed);
}

bool_t drive_segs(const vehicle_pos_t *pos, const vehicle_t *veh, list_t *segs,
                  double *last_mis_hdg, double d_t, double *out_steer,
                  double *out_speed, bool_t *out_decelerating) {
  ...
  if (seg->type == SEG_TYPE_STRAIGHT) {
    ...
    double len = vect2_dotprod(vect2_sub(fixed_pos, seg->start_pos), dir);
    double speed = straight_run_speed(veh, segs, seg->len - len, seg->backward,
                                      list_next(segs, seg), out_decelerating);
    double hdg =
        (!seg->backward ? seg->start_hdg : normalize_hdg(seg->start_hdg + 180));
    ...
    speed = (!seg->backward ? speed : -speed);
    drive_on_line(pos, veh, seg->start_pos, hdg, speed, veh->wheelbase / 4, 2,
                  B_TRUE, last_mis_hdg, d_t, out_steer, out_speed);
  } else {
    double rhdg = fabs(rel_hdg(pos->hdg, seg->end_hdg));
    double end_hdg =
        (!seg->backward ? seg->end_hdg : normalize_hdg(seg->end_hdg + 180));
    double end_brg =
        fabs(rel_hdg(end_hdg, dir2hdg(vect2_sub(fixed_pos, seg->end_pos))));
    double speed =
        turn_run_speed(veh, segs, ABS(rhdg), seg->turn.r, seg->backward,
                       list_next(segs, seg), out_decelerating);
    ...
    turn_run(pos, veh, seg, last_mis_hdg, d_t, speed, out_steer, out_speed);
  }
...
void seg_local2world(seg_t *seg) {
  ...
  if (XPLMProbeTerrainXYZ(probe, seg->start_pos.x, 0, -seg->start_pos.y,
                          &info)) {
    logMsg(BP_WARN_LOG
  ...
  if (XPLMProbeTerrainXYZ(probe, seg->end_pos.x, 0, -seg->end_pos.y, &info)) {
    logMsg(BP_WARN_LOG
           "XPLMProbeTerrainXYZ != xplm_ProbeHitTerrain  continuing anyway");
    XPLMDestroyProbe(probe);
    return;
...
void route_seg_append(avl_tree_t *route_table, route_t *r, const seg_t *seg) {
  ...
  if (list_head(&r->segs) == NULL) {
    ...
    r->pos_ecef = geo2ecef_mtr(GEO_POS3(r->pos.lat, r->pos.lon, 0), &wgs84);
...
static int route_table_compar(const void *a, const void *b) {
    ...
    if (strcmp(word, "route") == 0) {
      ...
      if (r == NULL) {
        logMsg(BP_ERROR_LOG "error parsing %s: 'seg' keyword must "
                            "follow a 'route' keyword.",
               filename);
        goto out;
      }
      if (fscanf(fp, "%u", &seg.type) != 1 || seg.type > SEG_TYPE_TURN) {
      ...
      seg.have_world_coords = B_TRUE;
      if (fscanf(fp, "%lf %lf %lf %lf %lf %lf %u", &seg.start_pos_geo.lat,
                 &seg.start_pos_geo.lon, &seg.start_hdg, &seg.end_pos_geo.lat,
                 &seg.end_pos_geo.lon, &seg.end_hdg, &seg.backward) != 7 ||
          !is_valid_hdg(seg.start_hdg) || !is_valid_hdg(seg.end_hdg)) {
        logMsg("[BP]ERROR: Error parsing %s: bad coordinates "
               "following 'seg' keyword",
               filename);
        goto out;
      }
      switch (seg.type) {
      case SEG_TYPE_STRAIGHT: {
        vect3_t start_ecef = geo2ecef_mtr(
            GEO_POS3(seg.start_pos_geo.lat, seg.start_pos_geo.lon, 0), &wgs84);
        vect3_t end_ecef = geo2ecef_mtr(
            GEO_POS3(seg.end_pos_geo.lat, seg.end_pos_geo.lon, 0), &wgs84);
      ...
      case SEG_TYPE_TURN:
        if (fscanf(fp, "%lf %u %u", &seg.turn.r, &seg.turn.right,
                   &seg.user_placed) != 3) {
...
static bool_t routes_store(avl_tree_t *t) {
  char *filename = mkpathname(ROUTE_TABLE_DIRS, ROUTE_TABLE_FILENAME, NULL);
 ...
 */
bool_t route_table_store(avl_tree_t *t, const char *filename) {
  ...
  if (sep != NULL) {
    *sep = 0;
    if (!file_exists(dirname, NULL) && !create_directory_recursive(dirname)) {
      free(dirname);
      return (B_FALSE);
    }
  }
  ...
  for (route_t *r = avl_first(t); r != NULL; r = AVL_NEXT(t, r)) {
    ...
    for (seg_t *seg = list_head(&r->segs); seg != NULL;
      ...
      fprintf(fp,
              "  seg %u %.17f %.17f %.1f %.17f %.17f "
              "%.1f %u ",
              seg->type, seg->start_pos_geo.lat, seg->start_pos_geo.lon,
              seg->start_hdg, seg->end_pos_geo.lat, seg->end_pos_geo.lon,
              seg->end_hdg, seg->backward);
      if (seg->type == SEG_TYPE_STRAIGHT) {
        fprintf(fp, "%u\n", seg->user_placed);
      } else {
        ASSERT3U(seg->type, ==, SEG_TYPE_TURN);
        fprintf(fp, "%.3f %u %u\n", seg->turn.r, seg->turn.right,
                seg->user_placed);
      }
    }
  }
...
static void routes_free(avl_tree_t *t) {
...
void route_load(geo_pos2_t start_pos, double start_hdg, list_t *segs) {
  ...
  srch.pos = start_pos;
  srch.pos_ecef =
      geo2ecef_mtr(GEO_POS3(start_pos.lat, start_pos.lon, 0), &wgs84);
  srch.hdg = start_hdg;
