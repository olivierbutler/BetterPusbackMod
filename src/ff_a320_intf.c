...
static inline int32_t gets32(int id) {
  int val;
  unsigned int type = svi.ValueType(id);
  ASSERT_MSG(type >= Value_Type_sint8 && type <= Value_Type_uint32,
             "%s isn't an integer type, instead it is %s", svi.ValueName(id),
             type2str(type));
  svi.ValueGet(id, &val);
  return (val);
}
...
static inline void sets32(int id, int32_t val) {
  unsigned int type = svi.ValueType(id);
  ASSERT_MSG(type >= Value_Type_sint8 && type <= Value_Type_uint32,
             "%s isn't an integer type, instead it is %s", svi.ValueName(id),
             type2str(type));
...
static void units2str(unsigned int units, char buf[32]) {
  snprintf(buf, 32,
...
static int val_id(const char *name) { return (svi.ValueIdByName(name)); }

static void ff_a320_ids_init(void) {
  if (ids.inited)
    return;

  ids.acf = val_id("Aircraft");

  ids.start = svi.ValueObjectNewValue(
      ids.acf, "BetterPushbackStart", "Start pushback operation", &cmds.start,
      Value_Type_uint32, Value_Flag_External, Value_Unit_State);
  ids.can_start = svi.ValueObjectNewValue(
      ids.acf, "BetterPushbackCanStart",
      "Can the pushback operation be started now?", &cmds.can_start,
      Value_Type_uint32, Value_Flag_External, Value_Unit_State);

  ids.stop = svi.ValueObjectNewValue(
      ids.acf, "BetterPushbackStop", "Stop pushback operation", &cmds.stop,
      Value_Type_uint32, Value_Flag_External, Value_Unit_State);
  ids.can_stop = svi.ValueObjectNewValue(
      ids.acf, "BetterPushbackCanStop",
      "Can the pushback operation be stopped now?", &cmds.can_stop,
      Value_Type_uint32, Value_Flag_External, Value_Unit_State);

  ids.start_planner = svi.ValueObjectNewValue(
      ids.acf, "BetterPushbackStartPlanner", "Start pushback path planner",
      &cmds.start_planner, Value_Type_uint32, Value_Flag_External,
      Value_Unit_State);
  ids.can_start_planner = svi.ValueObjectNewValue(
      ids.acf, "BetterPushbackCanStartPlanner",
      "Can the pushback planner be started now?", &cmds.can_start_planner,
      Value_Type_uint32, Value_Flag_External, Value_Unit_State);

  ids.cab_camera = svi.ValueObjectNewValue(
      ids.acf, "BetterPushbackCabCamera",
      "Switch to view from pushback tug cab", &cmds.cab_camera,
      Value_Type_uint32, Value_Flag_External, Value_Unit_State);
  ids.can_cab_camera = svi.ValueObjectNewValue(
      ids.acf, "BetterPushbackCanCabCamera",
      "Can the view be switched to the tug cab now?", &cmds.can_cab_camera,
      Value_Type_uint32, Value_Flag_External, Value_Unit_State);
...
static float cmd_dispatch(float elapsed, float elapsed2, int counter,
  ...
  if (cmds.start_planner) {
    XPLMCommandOnce(XPLMFindCommand("BetterPushback/start_planner"));
