...
static anim_info_t anim[TUG_NUM_ANIMS] = {
    {.name = "bp/anim/front_drive"},        {.name = "bp/anim/front_steer"},
    {.name = "bp/anim/rear_drive"},         {.name = "bp/anim/lift"},
    {.name = "bp/anim/lift_arm"},           {.name = "bp/anim/tire_sense"},
    {.name = "bp/anim/vehicle_lights"},     {.name = "bp/anim/cradle_lights"},
    {.name = "bp/anim/reverse_lights"},     {.name = "bp/anim/hazard_lights"},
    {.name = "bp/anim/driver_orientation"}, {.name = "bp/anim/cab_position"},
    {.name = "bp/anim/winch_on"},           {.name = "bp/anim/clear_signal"},
    {.name = "bp/anim/lift_in_transit"},    {.name = "bp/anim/beacon_flash"},
    {.name = "bp/anim/tug_pos_x"},          {.name = "bp/anim/tug_pos_y"},
    {.name = "bp/anim/tug_pos_z"}};

static const char *tug_dr_list[TUG_NUM_ANIMS + 1] = {
    "bp/anim/front_drive",        "bp/anim/front_steer",
    "bp/anim/rear_drive",         "bp/anim/lift",
    "bp/anim/lift_arm",           "bp/anim/tire_sense",
    "bp/anim/vehicle_lights",     "bp/anim/cradle_lights",
    "bp/anim/reverse_lights",     "bp/anim/hazard_lights",
    "bp/anim/driver_orientation", "bp/anim/cab_position",
    "bp/anim/winch_on",           "bp/anim/clear_signal",
    "bp/anim/lift_in_transit",    "bp/anim/beacon_flash",
    "bp/anim/tug_pos_x",          "bp/anim/tug_pos_y",
    "bp/anim/tug_pos_z",          NULL};
...
static inline float anim_gate(float x) {
  while (x > 1.0)
    x -= 1.0;
  while (x < 0.0)
    x += 1.0;
 ...
 */
int liv_compar(const void *a, const void *b) {
 ...
 */
int tug_info_compar(const void *a, const void *b) {
...
static void tug_info_free(tug_info_t *ti) {
  free(ti->tug_name);
  free(ti->tugdir);
  free(ti->tug);
  free(ti->arpt);
...
static bool_t regex_setup(const char *regex_str, const char *filename,
  ...
  *re = pcre2_compile((PCRE2_SPTR)regex_str, PCRE2_ZERO_TERMINATED,
  ...
  if (*re == NULL) {
    ...
    logMsg(BP_ERROR_LOG "Malformed tug livery config %s: regex compile "
                        "failed \"%s\": char %d: %s",
           filename, regex_str, (int)erroff, buf);
    return (B_FALSE);
  }
...
static bool_t tug_info_liv_cfg_match(const char *info_pathname,
                                     const char *icao, const char *airline,
                                     bool_t *force_pick_this,
                                     bool_t *is_airline,
                                     bool_t *airline_matched,
                                     bool_t *airline_optional) {
  ...
  while (!feof(fp)) {
    ...
    if (strcmp(cmd, "icao") == 0) {
      ...
      if (regex_str == NULL || *regex_str == 0 ||
          !regex_setup(regex_str, info_pathname, &re, &match_data, 0)) {
        logMsg(BP_ERROR_LOG "Malformed tug livery config %s: bad "
                            "regex following \"icao\".",
               info_pathname);
        free(regex_str);
        break;
      }

      rc = pcre2_match(re, (PCRE2_SPTR)icao, strlen(icao), 0, 0, match_data,
                       NULL);
      ...
      free(regex_str);
    } else if (strcmp(cmd, "airline") == 0 || strcmp(cmd, "airlinei") == 0) {
      ...
      *is_airline = B_TRUE;
      ...
      if (regex_str == NULL || *regex_str == 0 ||
          !regex_setup(regex_str, info_pathname, &re, &match_data, flags)) {
        logMsg(BP_ERROR_LOG "Malformed tug livery config %s: bad "
      ...
      rc = pcre2_match(re, (PCRE2_SPTR)airline, strlen(airline), 0, 0,
                       match_data, NULL);
      pcre2_match_data_free(match_data);
      pcre2_code_free(re);
      if (rc >= 0) {
      ...
      *force_pick_this = B_TRUE;
      logMsg(BP_WARN_LOG "%s: present forcibly picking", info_pathname);
    } else {
...
static bool_t tug_info_liv_select(tug_info_t *ti, const char *icao,
                                  const char *airline) {
  bool_t isdir;
  char *livdir_path;
  avl_tree_t livs;
  ...
  if (icao != NULL && *icao != 0) {
    ...
    while ((de = readdir(livdir)) != NULL) {
      ...
      if (dot == NULL || strcmp(&dot[1], "livery") != 0 ||
        ...
        continue;
      info_pathname =
          mkpathname(ti->tugdir, "liveries", de->d_name, "info.cfg", NULL);
      matched = tug_info_liv_cfg_match(info_pathname, icao, airline,
                                       &force_pick_this, &is_airline,
                                       &airline_matched, &airline_optional);
      if (matched && (!is_airline || !airline_optional || !airline_matched ||
                      force_pick_this)) {
        ...
        if (force_pick_this) {
          ...
          cookie = NULL;
          while ((oth_liv = avl_destroy_nodes(&livs, &cookie)) != NULL) {
  ...
  if (avl_numnodes(&livs) != 0) {
    ...
    livdir_path = mkpathname(ti->tugdir, "liveries", "generic.livery", NULL);
...
static tug_info_t *tug_info_read(const char *tugdir, const char *tug_name,
                                 const char *icao, const char *airline) {
  tug_info_t *ti = NULL;
  ...
  if (fp == NULL) {
    logMsg(BP_ERROR_LOG "Malformed tug %s, cannot open info.cfg: %s", tugdir,
           strerror(errno));
    free(cfgfilename);
    return (NULL);
  }
...
#define VALIDATE_TUG_INT(field, optname) VALIDATE_TUG((field) <= 0, (optname))
...
static tug_info_t *tug_info_select(double mtow, double ng_len, double tirrad,
                                   unsigned gear_type, const char *arpt,
                                   const char *airline, char **reason) {
  char *tugdir;
  DIR *dirp;
  struct dirent *de;
  avl_tree_t tis;
  ...
  while ((de = readdir(dirp)) != NULL) {
    ...
    if ((dot = strrchr(de->d_name, '.')) == NULL || strcmp(&dot[1], "tug") != 0)
      continue;
    ...
    if (ti != NULL && (ti->anim_debug || ti->drive_debug || ti->quick_debug)) {
      free(tugpath);
      goto out;
    }
    /*
    ...
    if (ti != NULL && reason != NULL) {
      append_format(reason, &cap,
                    ...
                    "result=",
                    de->d_name, ti->min_mtow, ti->max_mtow, ti->min_tirrad,
                    ti->max_tirrad, ti->min_nlg_len, ti->gear_compat,
                    ti->arpt != NULL ? ti->arpt : "(nil)");
    }
    if (ti != NULL && ti->min_mtow <= mtow && mtow <= ti->max_mtow &&
        ti->min_nlg_len <= ng_len && ti->min_tirrad <= tirrad &&
        ti->max_tirrad >= tirrad &&
        (arpt == NULL || ti->arpt == NULL || strcmp(arpt, ti->arpt) == 0) &&
...
static int reload_tug_handler(XPLMCommandRef cmd, XPLMCommandPhase phase,
                              void *refcon) {
  UNUSED(cmd);
  UNUSED(refcon);
  ...
  if (glob_tug != NULL && !glob_tug->load_in_prog) {
    ...
    if (objpath == NULL) {
      logMsg(BP_ERROR_LOG "Error preparing tug object %s", glob_tug->info->tug);
      return (1);
    }
    glob_tug->tug = XPLMLoadObject(objpath);
    tug_liv_destroy(objpath);
    VERIFY(glob_tug->tug != NULL);
    glob_tug->instance = XPLMCreateInstance(glob_tug->tug, tug_dr_list);
  }
...
bool_t tug_glob_init(void) {
  ...
  for (anim_t a = 0; a < TUG_NUM_ANIMS; a++) {
    dr_create_f(&anim[a].dr, &anim[a].value, B_FALSE, "%s", anim[a].name);
  }

  fdr_find(&sun_pitch_dr, "sim/graphics/scenery/sun_pitch_degrees");

  tug_reload_cmd =
      XPLMCreateCommand("BetterPushback/reload_tug", "Reload tug model");
...
void tug_glob_fini(void) {
  ...
  if (tug_reload_cmd != NULL) {
    XPLMUnregisterCommandHandler(tug_reload_cmd, reload_tug_handler, 0, NULL);
    tug_reload_cmd = NULL;
  }
...
static void tug_liv_subst(const tug_info_t *ti, FILE *fp, const char *tex_type,
                          const char *tex_name) {
  char *pathname;
  bool_t exists;
  char *tex_name_dds, *dot;

  pathname = mkpathname(ti->tugdir, "liveries", ti->livname, tex_name, NULL);
  exists = file_exists(pathname, NULL);
  free(pathname);
  if (exists) {
    fprintf(fp, "%s liveries/%s/%s\n", tex_type, ti->livname, tex_name);
    return;
  }
  /*
  ...
  if (dot != NULL) {
    strcpy(&dot[1], "dds");
    pathname =
        mkpathname(ti->tugdir, "liveries", ti->livname, tex_name_dds, NULL);
    exists = file_exists(pathname, NULL);
    free(pathname);
  }
  free(tex_name_dds);
...
bool_t tug_available(double mtow, double ng_len, double tirrad,
  ...
  char *reason = NULL;
  tug_info_t *ti =
      tug_info_select(mtow, ng_len, tirrad, gear_type, arpt, airline, &reason);
...
static void tug_load_complete(XPLMCommandRef obj, void *refcon) {
  tug_t *tug;
...
static tug_t *tug_alloc_common(tug_info_t *ti, double tirrad) {
  tug_t *tug;
...
#define LOAD_TUG_SOUND(sound)                                                  \
  do {                                                                         \
    ...
    if (tug->sound == NULL) {                                                  \
      logMsg(BP_ERROR_LOG "Error loading tug sound %s", tug->info->sound);     \
  ...
  if (bp_xp_ver >= 11000) {
    ...
    fdr_find(&tug->int_vol, "sim/operation/sound/interior_volume_ratio");
  } else {
    fdr_find(&tug->ext_vol, "sim/operation/sound/engine_volume_ratio");
    fdr_find(&tug->int_vol, "sim/operation/sound/engine_volume_ratio");
  }

  /* Flight Factor A320 integration */
  if (dr_find(&tug->cockpit_window_drs[0], "model/controls/window_l")) {
    fdr_find(&tug->cockpit_window_drs[1], "model/controls/window_r");
    tug->num_cockpit_window_drs = 2;
    /* IXEG 737 integration */
  } else if (dr_find(&tug->cockpit_window_drs[0],
                     "ixeg/733/misc/pilot_window")) {
    fdr_find(&tug->cockpit_window_drs[1], "ixeg/733/misc/copilot_window");
    tug->num_cockpit_window_drs = 2;
...
tug_t *tug_alloc_man(const char *tug_name, double tirrad, const char *arpt,
                     const char *airline) {
  char *tug_path =
      mkpathname(bp_xpdir, bp_plugindir, "objects", "tugs", tug_name, NULL);
...
bool_t tug_drive2point(tug_t *tug, vect2_t dst, double hdg) {
  ...
  return (compute_segs(&tug->veh, cur_pos, cur_hdg, dst, hdg, &tug->segs) >= 0);
}
...
void tug_run(tug_t *tug, double d_t, bool_t drive_slow) {
  ...
  if (list_head(&tug->segs) != NULL) {
    drive_segs(&tug->pos, drive_slow ? &tug->veh_slow : &tug->veh, &tug->segs,
               &tug->last_mis_hdg, d_t, &steer, &speed, NULL);
  }

  /* modulate our speed based on required steering angle */
  if (speed > 0) {
    speed =
        MIN(speed, tug->veh.max_fwd_spd * (1.1 - (steer / tug->veh.max_steer)));
  } else {
    speed = MAX(speed,
                -tug->veh.max_rev_spd * (1.1 - (steer / tug->veh.max_steer)));
  }
  ...
  if (radius > -1e3 && radius < 1e3) {
    ...
    if (!tug->info->anim_debug) {
      ...
      tug->pos.pos =
          vect2_add(tug->pos.pos, vect2_scmul(dir, tug->pos.spd * d_t));
    }
  }
  ...
  if (!tug->TE_override) {
    ...
    while (gear > 0) {
      ...
      if (spd > max_gear_spd)
        break;
      TE = MAX((spd - min_gear_spd) / (max_gear_spd - min_gear_spd), 0);
      gear--;
    }
    /*
...
void tug_anim(tug_t *tug, double d_t, double cur_t) {
  ...
  if (!tug->info->anim_debug) {
    anim[ANIM_FRONT_DRIVE].value = anim_gate(
        ((tug->pos.spd * d_t) / (2 * M_PI * tug->info->front_radius)) +
        anim[ANIM_FRONT_DRIVE].value);

    anim[ANIM_REAR_DRIVE].value =
        anim_gate(((tug->pos.spd * d_t) / (2 * M_PI * tug->info->rear_radius)) +
                  anim[ANIM_REAR_DRIVE].value);

    anim[ANIM_FRONT_STEER].value =
        (tug->cur_steer / (2 * tug->veh.max_steer)) + 0.5;
    ...
    if (tug->pos.spd < -0.1) {
      ...
      anim[ANIM_DRIVER_ORIENTATION].value =
          MIN(1, anim[ANIM_DRIVER_ORIENTATION].value + d_t / DRIVER_TURN_TIME);
      anim[ANIM_CAB_POSITION].value =
          MIN(1, anim[ANIM_CAB_POSITION].value + d_t / CAB_LIFT_TIME);
    } else if (tug->pos.spd > 0.1) {
      anim[ANIM_REVERSE_LIGHTS].value = B_FALSE;
      anim[ANIM_DRIVER_ORIENTATION].value =
          MAX(0, anim[ANIM_DRIVER_ORIENTATION].value - d_t / DRIVER_TURN_TIME);
      anim[ANIM_CAB_POSITION].value =
          MAX(0, anim[ANIM_CAB_POSITION].value - d_t / CAB_LIFT_TIME);
    }
    anim[ANIM_CRADLE_LIGHTS].value =
        (cradle_lights_req && dr_getf(&sun_pitch_dr) < LIGHTS_ON_SUN_ANGLE);
  } else {
    float value;
  ...
  anim[ANIM_BEACON_FLASH].value =
      ((cur_t_ms % BEACON_FLASH_INTVAL) <= BEACON_FLASH_DUR &&
       anim[ANIM_HAZARD_LIGHTS].value);
}
...
void tug_draw(tug_t *tug, double cur_t) {
  ...
  if (XPLMProbeTerrainXYZ(probe, tug->pos.pos.x, 0, -tug->pos.pos.y, &info) !=
      xplm_ProbeHitTerrain) {
    logMsg(BP_WARN_LOG
           "XPLMProbeTerrainXYZ != xplm_ProbeHitTerrain  continuing anyway");
  ...
  if (dr_geti(&tug->sound_on) == 1) {
    vect3_t cam_pos_xp =
        VECT3(dr_getf(&tug->cam_x), dr_getf(&tug->cam_y), dr_getf(&tug->cam_z));
    ...
    if (tug->info->electric_drive) {
      ...
      if (tug->TE_override) {
        ...
        if (tug->pos.spd == 0) {
          ...
          x = (ABS(tug->pos.spd) / tug->info->max_fwd_speed) + 0.2;
        }
      }
      gain_in *= x;
    }
    ...
    for (unsigned i = 0; i < tug->num_cockpit_window_drs; i++) {
      window = MAX(dr_getf(&tug->cockpit_window_drs[i]), window);
    }
  ...
  if (tug->air_snd != NULL) {
    if (tug->cradle_air_on) {
      double ramp =
          MIN((cur_t - tug->cradle_air_chg_t) / TUG_CRADLE_CHG_D_T, 1);
      wav_set_gain(tug->air_snd, gain_out * ramp * TUG_CRADLE_AIR_MOD);
      wav_set_gain(tug->air_snd_in, gain_in * ramp * TUG_CRADLE_AIR_MOD);
      ...
      double ramp = 1 - ((cur_t - tug->cradle_air_chg_t) / TUG_CRADLE_CHG_D_T);
      wav_set_gain(tug->air_snd, gain_out * ramp * TUG_CRADLE_AIR_MOD);
      wav_set_gain(tug->air_snd_in, gain_in * ramp * TUG_CRADLE_AIR_MOD);
...
void tug_set_TE_override(tug_t *tug, bool_t flag) { tug->TE_override = flag; }
...
void tug_set_steering(tug_t *tug, double req_steer, double d_t) {
...
double tug_plat_h(const tug_t *tug) {
  if (tug->info->lift_type != LIFT_WINCH)
    return (0);
  return ((1 - (tug->tirrad / (tug->info->lift_wall_z - tug->info->plat_z))) *
          tug->info->plat_h);
}
...
void tug_set_lift_arm_pos(const tug_t *tug, float x, bool_t grabbing_tire) {
  ...
  if (grabbing_tire) {
    ...
    double xx =
        (tug->tirrad - ti->min_tirrad) / (ti->max_tirrad - ti->min_tirrad);
    x = MIN(MAX(xx, 0), 1);
...
void tug_set_cradle_lights_on(bool_t flag) { cradle_lights_req = flag; }
