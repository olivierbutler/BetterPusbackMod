...
inline bool ends_with(const TContainer &input, const TContainer &match) {
  return input.size() >= match.size() &&
         input.compare(input.size() - match.size(), match.size(), match) == 0;
}
...
#if APL == 1 || LIN == 1
inline void strncpy_s(char *dest, size_t destsz, const char *src,
                      size_t count) {
...
#define STRCPY_ATMOST(dest, src)                                               \
  strncpy_s(dest, sizeof(dest), strAtMost(src, sizeof(dest) - 1).c_str(),      \
            sizeof(dest) - 1)
...
inline std::string joinSkipFirst(const std::vector<std::string> &v,
...
inline int CountFilesInPath(const std::string &path) {
  char buffer[2048];
  int fileCount = 0;
  XPLMGetDirectoryContents(path.c_str(), 0, buffer, sizeof(buffer), nullptr, 0,
                           &fileCount, nullptr);
  return fileCount;
}
...
inline std::string UtcTimestamp() {
  ...
  char buffer[128];
  return std::string(
      buffer, buffer + strftime(buffer, sizeof(buffer), "%H:%M:%S", timeInfo));
}
...
inline const char *LOG_LEVEL[] = {" DEBUG ", " INFO ",  " WARN ",
                                  " ERROR ", " FATAL ", ""};
...
inline const char *Logger(logLevel level, const char *msg, va_list args) {
  ...
  snprintf(buf, sizeof(buf), "%u:%02u:%06.3f %s: %s", hours, mins, secs,
           PLUGIN_NAME, LOG_LEVEL[level]);
  if (args) {
