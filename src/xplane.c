...
static bool_t prefs_enable, stop_pb_plan_enable, stop_pb_enable,
    conn_first_enable, cab_cam_enable;
...
static int manual_push_reverse_handler(XPLMCommandRef, XPLMCommandPhase,
                                       void *);
static int manual_push_start_handler(XPLMCommandRef, XPLMCommandPhase, void *);
static int manual_push_start_no_yoke_handler(XPLMCommandRef, XPLMCommandPhase,
                                             void *);
static int manual_push_reverse_handler(XPLMCommandRef, XPLMCommandPhase,
                                       void *);
 ...
 */
static void set_xp11_tug_hidden(bool_t flag) {
  ...
  if (flag == hidden) {
    ...
    logMsg(BP_WARN_LOG "Hidden xp11 default tug not supported. XP Version %d",
           bp_xp_ver);
    return;
  }
  ...
  filename = mkpathname(bp_xpdir, "Resources", "default scenery", "sim objects",
                        "apt_vehicles", "pushback", "Tug_GT110.obj", NULL);
  ...
  if (flag) {
    ...
    if (!file_exists(filename, NULL) || !file_exists(filename_backup, NULL)) {
...
static int start_pb_handler_(XPLMCommandRef cmd, XPLMCommandPhase phase,
                             void *refcon) {
  UNUSED(cmd);
  UNUSED(refcon);
  ...
  if (!start_pb_enable) {
    logMsg(BP_WARN_LOG
           "Command \"BetterPushback/start\" is currently disabled");
  ...
  stop_cam_handler(NULL, xplm_CommandEnd,
                   NULL); // synchronously stop a possible open cam
...
static int start_pb_handler(XPLMCommandRef cmd, XPLMCommandPhase phase,
                            void *refcon) {
  if (phase != xplm_CommandEnd)
    return (1);
  ...
  if (((tug_auto_start && tug_starts_next_plane) || always_connect_tug_first) &&
      !bp_started) {
...
static int manual_push_start_handler_(XPLMCommandRef cmd,
...
static int manual_push_start_handler(XPLMCommandRef cmd, XPLMCommandPhase phase,
                                     void *refcon) {
  if (phase != xplm_CommandEnd)
    return (1);
...
static int manual_push_start_no_yoke_handler(XPLMCommandRef cmd,
...
static int stop_pb_handler(XPLMCommandRef cmd, XPLMCommandPhase phase,
                           void *refcon) {
  UNUSED(cmd);
  UNUSED(refcon);
...
static int manual_push_reverse_handler(XPLMCommandRef cmd,
                                       XPLMCommandPhase phase, void *refcon) {
  UNUSED(cmd);
  UNUSED(refcon);
  if (phase != xplm_CommandEnd)
  ...
  if (push_manual.active) {
    if ((bp_ls.tug->pos.spd > 0.1) || (bp_ls.tug->pos.spd < -0.1)) {
      logMsg("Manual push:  in progress, tug is moving to fast toggling "
             "direction is not yet possibe");
      return (1);
    }
    push_manual.forward_direction = !push_manual.forward_direction;
    logMsg("Manual push:  Toggling direction to %s",
           push_manual.forward_direction ? "forward" : "backward");
  } else {
...
static int start_cam_handler(XPLMCommandRef cmd, XPLMCommandPhase phase,
                             void *refcon) {
  UNUSED(cmd);
  UNUSED(refcon);
  ...
  if (!start_pb_plan_enable) {
    logMsg(BP_WARN_LOG
           "Command \"BetterPushback/start_planner\" is currently disabled");
    return (1);
  }
...
static int stop_cam_handler(XPLMCommandRef cmd, XPLMCommandPhase phase,
                            void *refcon) {
  UNUSED(cmd);
  UNUSED(refcon);
  ...
  if (!stop_pb_plan_enable) {
    logMsg(BP_WARN_LOG
           "Command \"BetterPushback/stop_planner\" is currently disabled");
    return (1);
  }
...
static int conn_first_handler(XPLMCommandRef cmd, XPLMCommandPhase phase,
                              void *refcon) {
  UNUSED(cmd);
  UNUSED(refcon);
  ...
  if (!conn_first_enable) {
    logMsg(BP_WARN_LOG
           "Command \"BetterPushback/connect_first\" is currently disabled");
    return (0);
  }
  ...
  /*
   * The conn_first procedure results in 2 calls to bp_start(). First here to
   * get the tug connected, then the second is issued by the user to get things
   * moving. An *active* preplanned route interferes with the holding point
   * after lift in pb_step_lift() / late_plan_end_cond() . So we save an active
   * preplanned route here and clear it. It will be loaded again when the
   * planner is started for the final review in the user invocation of
   * bp_start() so the planning is not lost but it may be modified.
   */
  if (bp_num_segs()) {
    route_save(&bp.segs);
    bp_delete_all_segs();
...
static int cab_cam_handler(XPLMCommandRef cmd, XPLMCommandPhase phase,
                           void *refcon) {
  UNUSED(cmd);
  UNUSED(refcon);
  if (phase != xplm_CommandEnd)
  ...
  if (!cab_cam_enable) {
    logMsg(BP_WARN_LOG
           "Command \"BetterPushback/cab_cam\" is currently disabled");
    return (0);
  }

  if (!cab_view_start()) {
    XPLMSpeakString(
        _("ERROR: Unable to select pushback tug view at this time."));
    return (0);
  }
...
static int recreate_routes_handler(XPLMCommandRef cmd, XPLMCommandPhase phase,
                                   void *refcon) {
  UNUSED(cmd);
  UNUSED(refcon);
  if (phase != xplm_CommandEnd)
...
static int preference_window_handler(XPLMCommandRef cmd, XPLMCommandPhase phase,
                                     void *refcon) {
  UNUSED(cmd);
  UNUSED(refcon);
  if (phase != xplm_CommandEnd)
  ...
  if (!prefs_enable) {
    logMsg(BP_WARN_LOG
           "Command \"BetterPushback/preference\" is currently disabled");
    return (0);
  }
...
static void menu_cb(void *inMenuRef, void *inItemRef) {
  UNUSED(inMenuRef);
  if (inItemRef == NULL)
    return;
  else
...
static void coupled_state_change() {
...
static float status_check(float elapsed, float elapsed2, int counter,
                          void *refcon) {
  UNUSED(elapsed);
  UNUSED(elapsed2);
  UNUSED(counter);
  ...
  if (smartcopilot_present &&
      dr_geti(&smartcopilot_state) == COUPLED_STATE_SLAVE && !slave_mode) {
    if (bp_started) {
    ...
    slave_mode = B_TRUE;
    coupled_state_change();
  } else if (sharedflight_present &&
             dr_geti(&sharedflight_state) == COUPLED_STATE_SLAVE &&
             !slave_mode) {
    if (bp_started) {
    ...
    slave_mode = B_TRUE;
    coupled_state_change();
  } else if ((smartcopilot_present &&
              dr_geti(&smartcopilot_state) != COUPLED_STATE_SLAVE) &&
             (!sharedflight_present ||
              dr_geti(&sharedflight_state) != COUPLED_STATE_SLAVE) &&
             slave_mode) {
    if (bp_started) {
      XPLMSpeakString(_("Pushback failure: smartcopilot "
    ...
    coupled_state_change();
              ...
              dr_geti(&sharedflight_state) != COUPLED_STATE_SLAVE &&
              dr_geti(&sharedflight_state) != COUPLED_STATE_PASSENGER) &&
             (!smartcopilot_present ||
              dr_geti(&smartcopilot_state) != COUPLED_STATE_SLAVE) &&
             slave_mode) {
    if (bp_started) {
      XPLMSpeakString(_("Pushback failure: Shared Flight "
...
static void xlate_init(void) {
  char *po_file = mkpathname(xpdir, plugindir, "data", "po", bp_get_lang(),
                             "strings.po", NULL);
...
PLUGIN_API int XPluginStart(char *name, char *sig, char *desc) {
  ...
  logMsg(BP_INFO_LOG "This is Better Pushback (MOD) -" BP_PLUGIN_VERSION
                     " libacfutils-%s - for X-Plane 11/12",
         libacfutils_version);
  ...
  if (err != GLEW_OK) {
    ...
    logMsg(BP_FATAL_LOG "cannot initialize libGLEW: %s",
  ...
  if (strstr(plugindir, xpdir) == plugindir) {
    ...
    memmove(plugindir, &plugindir[xpdir_len], plugindir_len - xpdir_len + 1);
  }
  ...
  start_pb = XPLMCreateCommand("BetterPushback/start", _("Start pushback"));
  stop_pb = XPLMCreateCommand("BetterPushback/stop", _("Stop pushback"));
  ...
  conn_first =
      XPLMCreateCommand("BetterPushback/connect_first",
                        _("Connect tug before entering pushback plan"));
  cab_cam =
      XPLMCreateCommand("BetterPushback/cab_camera", _("View from tug's cab."));
  recreate_routes =
      XPLMCreateCommand("BetterPushback/recreate_scenery_routes",
                        _("Recreate scenery routes from WED files."));
  pref_cmd = XPLMCreateCommand("BetterPushback/preference",
  ...
  manual_push_reverse =
      XPLMCreateCommand("BetterPushback/manual_push_toggle",
                        _("Toggle the trajectory of the push back"));
  manual_push_start =
      XPLMCreateCommand("BetterPushback/manual_push_start",
                        _("Start/Pause the manual push back with yoke"));
  manual_push_start_no_yoke =
      XPLMCreateCommand("BetterPushback/manual_push_start_no_yoke",
                        _("Start/Pause the manual push back (yoke not used)"));

  bp_boot_init();

  dr_create_i(&bp_started_dr, (int *)&bp_started, B_FALSE, "bp/started");
  dr_create_i(&bp_connected_dr, (int *)&bp_connected, B_FALSE, "bp/connected");
  ...
  dr_create_i(&op_complete_dr, (int *)&op_complete, B_TRUE, "bp/op_complete");
  dr_create_i(&plan_complete_dr, (int *)&plan_complete, B_TRUE,
              "bp/plan_complete");
  dr_create_b(&bp_tug_name_dr, bp_tug_name, sizeof(bp_tug_name), B_TRUE,
              "bp/tug_name");

  pb_set_remote_dr_cfg.write_cb = pb_set_remote_cb;
  pb_set_remote_dr_cfg.writable = B_FALSE;
  dr_create_i_cfg(&pb_set_remote_dr, (int *)&pb_set_remote,
                  pb_set_remote_dr_cfg, "bp/parking_brake_set");
  ...
  dr_create_i_cfg(&pb_set_override_dr, (int *)&pb_set_override,
                  pb_set_override_dr_cfg, "bp/parking_brake_override");
 ...
 */
PLUGIN_API int XPluginEnable(void) { return (bp_priv_enable()); }

PLUGIN_API void XPluginDisable(void) { bp_priv_disable(); }
...
PLUGIN_API void XPluginReceiveMessage(XPLMPluginID from, int msg, void *param) {
  ...
  case XPLM_MSG_PLANE_LOADED:
    ...
    sharedflight_present =
        dr_find(&sharedflight_state, "SharedFlight/is_pilot_flying");
    bp_fini();
    cab_view_fini();
#ifndef SLAVE_DEBUG
    bp_tug_name[0] = '\0';
...
static bool_t bp_priv_enable(void) {
  ...
  XPLMRegisterCommandHandler(recreate_routes, recreate_routes_handler, 1, NULL);
  XPLMRegisterCommandHandler(pref_cmd, preference_window_handler, 1, NULL);
  XPLMRegisterCommandHandler(abort_push, abort_push_handler, 1, NULL);

  XPLMRegisterCommandHandler(manual_push_left, manual_push_left_handler, 1,
                             NULL);
  XPLMRegisterCommandHandler(manual_push_right, manual_push_right_handler, 1,
                             NULL);
  XPLMRegisterCommandHandler(manual_push_reverse, manual_push_reverse_handler,
                             1, NULL);
  XPLMRegisterCommandHandler(manual_push_start, manual_push_start_handler, 1,
                             NULL);
  XPLMRegisterCommandHandler(manual_push_start_no_yoke,
                             manual_push_start_no_yoke_handler, 1, NULL);

  plugins_menu_item =
      XPLMAppendMenuItem(XPLMFindPluginsMenu(), "Better Pushback", NULL, 1);
  root_menu = XPLMCreateMenu("Better Pushback", XPLMFindPluginsMenu(),
                             plugins_menu_item, menu_cb, NULL);

  start_pb_plan_menu_item = XPLMAppendMenuItemWithCommand(
      root_menu, _("Pre-plan pushback"), start_cam);
  stop_pb_plan_menu_item = XPLMAppendMenuItemWithCommand(
      root_menu, _("Close pushback planner"), stop_cam);
  conn_first_menu_item = XPLMAppendMenuItemWithCommand(
      root_menu, _("Connect tug first"), conn_first);
  start_pb_menu_item =
      XPLMAppendMenuItemWithCommand(root_menu, _("Start pushback"), start_pb);
  stop_pb_menu_item =
      XPLMAppendMenuItemWithCommand(root_menu, _("Stop pushback"), stop_pb);
  cab_cam_menu_item =
      XPLMAppendMenuItemWithCommand(root_menu, _("Tug cab view"), cab_cam);

  XPLMAppendMenuSeparator(root_menu);
  prefs_menu_item =
      XPLMAppendMenuItemWithCommand(root_menu, _("Preferences..."), pref_cmd);
...
static void bp_priv_disable(void) {
  ...
  XPLMUnregisterCommandHandler(recreate_routes, recreate_routes_handler, 1,
                               NULL);
  XPLMUnregisterCommandHandler(abort_push, abort_push_handler, 1, NULL);
  XPLMUnregisterCommandHandler(manual_push_left, manual_push_left_handler, 1,
                               NULL);
  XPLMUnregisterCommandHandler(manual_push_right, manual_push_right_handler, 1,
                               NULL);
  XPLMUnregisterCommandHandler(manual_push_reverse, manual_push_reverse_handler,
                               1, NULL);
  XPLMUnregisterCommandHandler(manual_push_start, manual_push_start_handler, 1,
                               NULL);
  XPLMUnregisterCommandHandler(manual_push_start_no_yoke,
                               manual_push_start_no_yoke_handler, 1, NULL);
...
static float bp_do_reload(float u1, float u2, int u3, void *u4) {
  UNUSED(u1);
  UNUSED(u2);
  UNUSED(u3);
  UNUSED(u4);
...
static int abort_push_handler(XPLMCommandRef cmd, XPLMCommandPhase phase,
                              void *refcon) {
  UNUSED(cmd);
  UNUSED(refcon);
  if (phase != xplm_CommandEnd)
...
#define STEER_MAX 100
  ...
  if (push_manual.active) {
    if (push_manual.with_yoke) {
      logMsg("Manual push:  Manual nose tug rotation disabled (yoke support "
             "enabled)");
      return;
    }
    float angle = to_the_left ? push_manual.angle - STEER_INCR
                              : push_manual.angle + STEER_INCR;
    ...
    logMsg("Manual push: Manual nose tug rotation disabled (manual push not "
           "active)");
  }
}

static int manual_push_left_handler(XPLMCommandRef cmd, XPLMCommandPhase phase,
                                    void *refcon) {
  UNUSED(cmd);
  UNUSED(refcon);
  if (phase != xplm_CommandEnd)
...
static int manual_push_right_handler(XPLMCommandRef cmd, XPLMCommandPhase phase,
                                     void *refcon) {
  UNUSED(cmd);
  UNUSED(refcon);
  if (phase != xplm_CommandEnd)
...
bool_t get_pref_widget_status(void) { return pref_widget_active_status; }
